        ...
        tinyMCE.init({        // General options
            ...
            setup: function (ed) {
                ed.onInit.add(function (ed) {
                    debugger;
                    $(ed.getBody()).find('img').bind('resize', function (event) {
                        ed.handler_array.push(event);
                        if (ed.firstResize == 0) {

                            //alert('set timeout!');

                            setTimeout(function () {
                                event = ed.handler_array.pop();
                                //alert (  event.target.width +','+ event.target.aspectratio );
                                $(event.target).css('width', parseInt(event.target.width * event.target.aspectratio));
                                ed.handler_array = [];
                            ...
                            setTimeout(function () {
                                ed.firstResize = 0;
                            }, 2000); //1000 is time in miliseconds
...
In a normal textarea - wrap=hard solves this, then using <pre> *text here*</pre> on the read page and it keeps the same formatting as the user entered.
        ...
        <img src="<%=ResolveUrl("~") %>Content/Tulips.jpg" class="" alt="" /> 
